...
| [cpp23_stdfloat.cpp](cpp23_stdfloat.cpp) | c++23 | GCC >= 13 (GCC only) | covered | <stdfloat> defines fixed-width floating types: std::float32_t/float64_t are guaranteed IEEE binary32/64 (unlike float/double, which only usually are), plus float16_t/bfloat16_t where the target has them. |
