...
_Folder: `features/gccext/sanitize/ubsan/`. 5 topic(s). Generated from `gcc_feature_test()` metadata and each file's `// description:` line; regenerate with `./scripts/container-dev.sh <ver> readme`._
...
| [gccext_ubsan_invalid_vptr.cpp](gccext_ubsan_invalid_vptr.cpp) | c++17 | GCC >= 13 (GCC only) | covered | UBSan's vptr check diagnoses a virtual call through storage that does not contain an object of the claimed dynamic type. |
