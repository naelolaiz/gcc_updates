...
The demo files set `requires-sanitizer=...` in their metadata; the discovery
engine skips them in regular runs and only enables them in matching sanitizer
runs. Their `expect-exit=` is set to the sanitizer's documented abort code so
we can assert the *correct kind of failure* rather than just "anything went
wrong".
