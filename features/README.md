...
Every `.cpp` here is a single-file program with a `// gcc-test:` metadata
header. The tree is split three ways:
...
Each leaf folder has its own auto-generated `README.md` listing its examples
grouped by topic. The narrative version of "what each release shipped" lives
in [../docs/gcc-changelogs.md](../docs/gcc-changelogs.md). The flag reference
is in [../docs/compiler-flags.md](../docs/compiler-flags.md).
